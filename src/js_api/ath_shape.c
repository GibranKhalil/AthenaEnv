...
static JSValue js_prim_list_ctor(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (argc > 3) {
        ...
        list_obj->texture = ((AthenaImage*)JS_GetOpaque2(ctx, argv[3], get_img_class_id()))->tex;
    }
...
static JSValue js_prim_list_draw(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
	float x, y;
...
static const JSCFunctionListEntry js_prim_list_proto_funcs[] = {
    JS_CFUNC_DEF("draw", 2, js_prim_list_draw),
};
...
static const JSCFunctionListEntry module_funcs[] = {
    ...
    JS_CFUNC_DEF("list", 4, js_prim_list_ctor),
...
static int js_draw_module_init(JSContext *ctx, JSModuleDef *m) {
    ...
    JS_SetPropertyFunctionList(ctx, proto, js_prim_list_proto_funcs, countof(js_prim_list_proto_funcs));
